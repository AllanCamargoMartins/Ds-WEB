...
switch ($method) {
    ...
    case 'POST':
        ...
        $preco = $body['preco'] ?? 0;
        $imagem_url = trim($body['imagem_url'] ?? '');
        $status = trim($body['status'] ?? 'Sonho');
        ...
        $database->executeQuery(
            'INSERT INTO itens (nome, categoria_id, preco, imagem_url, status) 
             VALUES (:nome, :categoria_id, :preco, :imagem_url, :status)',
            [
                ':nome' => $nome, 
    ...
    case 'PUT':
        ...
        if (!$status) {
            http_response_code(400);
            echo json_encode(['status' => 'error', 'message' => 'Status não informado.']);
            break;
        }

        $database->executeQuery('UPDATE itens SET status = :status WHERE id = :id', [':status' => $status, ':id' => $id]);
